...
const ServiceDialog = ({
  ...
  const mutation = useMutation({
    ...
    onError: (err: any) => { console.error("Service save error:", JSON.stringify(err, null, 2)); toast.error(`Erro ao salvar serviço: ${err?.message || err}`); },
  });
  ...
  const handleDeleteClick = async () => {
    if (!editing) return;
    // Check for appointments using this service
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (price === "" || price === null || price === undefined) { toast.error("O preço é obrigatório."); return; } mutation.mutate(); }}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {/* RIGHT COLUMN — Options, Obligations, Sessions */}
            <div className="space-y-4">
              {/* Advanced options */}
              <div className="space-y-3">
                <p className="text-xs uppercase tracking-wider text-muted-foreground font-medium">Opções Avançadas</p>
                <div className="flex items-center justify-between">
                  <Label className="text-sm">Disponível na página de marcações</Label>
                  <Switch checked={showOnBooking} onCheckedChange={setShowOnBooking} />
                </div>
                <div className="flex items-center justify-between">
                  <Label className="text-sm">Mostrar preço na página de marcações</Label>
              ...
              <div className="border-t border-border pt-4 space-y-3">
                <p className="text-xs uppercase tracking-wider text-muted-foreground font-medium">Obrigações do Serviço</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={consentPolicy} onValueChange={(v) => setConsentPolicy(v as any)} disabled={reqPhotos && consentPolicy === "none"}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">Não pedir consentimento</SelectItem>
                      <SelectItem value="once">Pedir na 1ª sessão (Estética Avançada)</SelectItem>
                      <SelectItem value="always">Pedir em todas as sessões (Medicina Estética)</SelectItem>
                    </SelectContent>
                  </Select>
                ...
                {multiSession && (
                  <div className="space-y-1.5 pl-6">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      O consentimento assinado na 1ª sessão será herdado automaticamente pelas {sessionCount ? Number(sessionCount) - 1 : "restantes"} sessões seguintes.
                    </p>
                  </div>
                )}
              </div>
