...
import {
  DollarSign, TrendingUp, Clock, CheckCircle, FileSpreadsheet,
  ArrowUpDown, ArrowUp, ArrowDown, ShoppingBag, Filter, X,
} from "lucide-react";
...
export default function FinancialReport({ dateRange }: Props) {
  ...
  // ── Base filtrada — alimenta TUDO: cards, gráficos e tabela ──────────────
  const filteredCharges = useMemo(() => {
    if (!charges) return [];
    return charges.filter((c) => {
      if (statusFilter !== "all" && c.status !== statusFilter) return false;
      if (paymentFilter !== "all" && (c as any).payment_method !== paymentFilter) return false;
      return true;
    });
  }, [charges, statusFilter, paymentFilter]);

  const hasActiveFilter = statusFilter !== "all" || paymentFilter !== "all";

  // ── KPI metrics ───────────────────────────────────────────────────────────
  const metrics = useMemo(() => {
    const paid = filteredCharges.filter((c) => c.status === "pago");
    const pending = filteredCharges.filter((c) => c.status === "pendente");
  ...
  const monthlyData = useMemo(() => {
    ...
    filteredCharges.filter((c) => c.status === "pago" && c.paid_at).forEach((c) => {
  ...
  const byService = useMemo(() => {
    const map = new Map<string, number>();
    filteredCharges.filter((c) => c.status === "pago").forEach((c) => {
  ...
  // ── Gráfico pizza serviços vs produtos ───────────────────────────────────
  ...
  const handleExport = () => {
    exportToXls("Financeiro", sortedCharges.map((c) => {
      const pm = (c as any).payment_method;
      return {
        Cliente: (c.clients as any)?.full_name || "—",
        Valor: Number(c.amount),
        Status: c.status,
        "Forma Pgto.": PAYMENT_METHODS.find((m) => m.value === pm)?.label ?? (pm || "—"),
  ...
  const getPaymentBadge = (status: string) => {
    if (status === "pago") return <span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 text-[10px] font-semibold"><CheckCircle className="w-3 h-3" />Pago</span>;
    if (status === "pendente") return <span className="inline-flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2 py-0.5 text-[10px] font-semibold"><Clock className="w-3 h-3" />Pendente</span>;
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="ml-auto text-xs text-muted-foreground">
            ...
            {hasActiveFilter && charges && charges.length !== filteredCharges.length && (
              <span className="text-muted-foreground/60"> de {charges.length}</span>
            )}
          </span>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: "Receita Total", value: fmt(metrics.totalRevenue), icon: DollarSign },
          { label: "Serviços", value: fmt(metrics.serviceRevenue), icon: TrendingUp },
          { label: "Produtos", value: fmt(metrics.productRevenue), icon: ShoppingBag },
          { label: "Cobranças Pagas", value: String(metrics.paidCount), icon: CheckCircle },
          { label: "Pendentes", value: fmt(metrics.totalPending), icon: Clock },
        ].map((kpi) => (
        ...
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={monthlyData}>
              ...
              <YAxis tick={{ fontSize: 11, fill: "hsl(30, 8%, 50%)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} />
          ...
          <Card className="p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={byService} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(30, 15%, 88%)" />
                <XAxis type="number" tick={{ fontSize: 11, fill: "hsl(30, 8%, 50%)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} />
        ...
        <div className="overflow-auto max-h-[400px]">
          <Table>
            ...
            <TableBody>
              ...
              ) : sortedCharges.slice(0, 100).map((c) => {
                const pm = (c as any).payment_method;
                const pmLabel = PAYMENT_METHODS.find((m) => m.value === pm)?.label ?? (pm || "—");
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{pmLabel}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
