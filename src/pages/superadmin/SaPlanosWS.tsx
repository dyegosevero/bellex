...
export default function SaPlanosWS() {
  return (
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      <div className="mb-1">
        <h1 className="text-lg font-semibold text-white/90">Planos WS</h1>
        <p className="text-[12px] text-white/30 mt-0.5">Planos que você oferece para novos operadores de Workspace (Modelo 2 — White-Label)</p>
      </div>

      <div className="rounded-xl border border-white/[0.07] bg-amber-950/20 border-amber-900/40 p-4 text-sm text-amber-300/70">
      ...
      <div className="grid md:grid-cols-3 gap-5">
        {WS_PLANS.map(p => (
          <div key={p.name} className="relative rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            {p.popular && (
              <div className="absolute top-3.5 right-3.5">
                <span className="text-[10px] font-bold bg-white/10 text-white/70 border border-white/10 px-2 py-0.5 rounded-full">Popular</span>
              </div>
            )}
            <div className="h-1" style={{ background: p.color }} />
            <div className="p-5 space-y-4">
              <div>
                <p className="text-base font-bold text-white/90">{p.name}</p>
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2.5">
                  <span className="text-[11px] text-white/40 flex items-center gap-1"><Building2 className="w-3 h-3" />{p.seats} seats</span>
                  <span className="text-[11px] text-white/40 flex items-center gap-1"><Zap className="w-3 h-3" />{p.ai_conversations.toLocaleString()} conv. IA</span>
                  <span className="text-[11px] text-white/40 flex items-center gap-1"><HardDrive className="w-3 h-3" />{p.storage_gb} GB</span>
                </div>
              </div>
