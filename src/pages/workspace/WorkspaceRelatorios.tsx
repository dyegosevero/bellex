import { BarChart3, Download, Building2, TrendingUp, DollarSign } from "lucide-react";
...
export default function WorkspaceRelatorios() {
  ...
  const stats = useMemo(() => {
    ...
    const byStatus = (["ativo","trial","inadimplente","suspenso","cancelado"] as const).map(s => ({
      name: s,
      count: myClinics.filter(c => c.status === s).length,
    }));
  ...
  const exportCsv = () => {
    const rows = [
      ["Cliente","Clínica","Subdomínio","Status","Plano","Valor/mês","Criado em"],
      ...myClinics.map(c => [
        c.client_name ?? "",
        c.name,
        c.subdomain,
        c.status,
        c.plan,
        `R$ ${(planPrice[c.plan] ?? 0).toLocaleString("pt-BR")}`,
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-start justify-between">
        <PageHeader icon={<BarChart3 className="w-5 h-5" />} title="Relatórios" subtitle="Visão geral por clínica" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          { icon: Building2, label: "Total de clínicas", value: String(myClinics.length), sub: `${stats.activeClinics.length} ativas`, color: "#e8957a" },
          { icon: TrendingUp, label: "Clínicas ativas", value: String(stats.activeClinics.length), sub: `${myClinics.filter(c=>c.status==="trial").length} em trial`, color: "#60a5fa" },
          { icon: DollarSign, label: "MRR", value: `R$ ${stats.mrr.toLocaleString("pt-BR")}`, sub: "clínicas ativas", color: "#34d399" },
        ].map(k => (
        ...
        <div className="rounded-2xl border border-border/40 bg-card p-5 space-y-3">
          ...
          <div className="space-y-2.5 pt-2">
            {stats.byStatus.map(s => {
              const pct = myClinics.length ? Math.round((s.count / myClinics.length) * 100) : 0;
            ...
            <tbody>
              {myClinics.map((c, i) => (
                <tr key={c.id} className={`border-b border-border/20 hover:bg-muted/10 ${i % 2 ? "bg-muted/5" : ""}`}>
                  ...
                  <td className="p-4 hidden sm:table-cell text-xs text-muted-foreground capitalize">{c.plan}</td>
